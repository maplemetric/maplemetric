...
표만 있고 비어 있는 백업으로 되돌리기 — 스키마는 같고 행이 없는 백업으로 시도했다.
수집 기준일이 0개라 멈췄고, 대상은 739행 그대로 남았다.

## 백업을 고를 때

이 스크립트는 되돌린 것에 표가 있고 수집 기준일이 하나 이상인지까지만 본다. 그것으로
막을 수 있는 것은 엉뚱한 데이터베이스의 백업과 비어 있는 백업이다.

같은 스키마에 데이터도 든, 그러나 원하던 것이 아닌 백업은 걸러 내지 못한다. 그래서
자리를 바꾸기 전에 행 수를 찍고 한 번 더 묻는다. 파일 이름의 날짜와 시각을 보고
고르고, 그 행 수가 예상과 맞는지 본 뒤에 `yes`를 입력한다.

무인으로 돌린다면 최소 기준일 수를 함께 준다.

```powershell
.\scripts\restore-database.ps1 -BackupFile <파일> -Force `
    -MinimumCollectionCount 700
```

## .env를 읽는 방식

값을 감싼 따옴표는 벗긴다. Compose가 그렇게 다루기 때문이다.

다른 값을 끌어다 쓰는 `${...}` 표기는 풀지 않고 거부한다. 반쯤 아는 채로 넘기면 틀린
