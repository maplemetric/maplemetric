...
확인을 바꾸기 전에 한다. `pg_restore`가 성공했다는 것은 파일을 읽었다는 뜻이지 그것이
맞는 백업이라는 뜻이 아니다. 다른 데이터베이스를 받아 둔 파일이거나 표가 빠져 있어도
되돌리기 자체는 끝난다. 확인이 바꾼 뒤에 있으면 그때는 되돌아갈 원본이 없다.
...
다른 데이터베이스의 백업으로 되돌리기 — 관계없는 표 하나만 든 백업으로 시도했다.
`pg_restore`는 성공했지만 확인에서 걸려 멈췄고, 대상은 739행 그대로 남았다.
